...
-- T02_ITEM, T02_ITEM_GROUP, T03_ITEM_DETAIL , T12_SIZE
...
-- SELECT @running := @running + 1 SID,
--        C01_ITEM_ID ITEM_ID,
--        CASE
-- 		WHEN @running <= 4 THEN 1
--         WHEN @running <= 8 THEN 2
--         WHEN @running <= 12 THEN 3
--         WHEN @running <= 16 THEN 4
--         WHEN @running <= 20 THEN 5
--        END AS SUPPLIER_ID,
--        elt((floor(rand() * 5) + 1), 220, 400, 582, 600, 300) AMOUNT,
--        DATE_SUB(current_timestamp(), INTERVAL (@max_item_id-@running) DAY) ENTERING_DATE,
...
-- T08 CUSTOMER, T11_TITLE, T13_EMPLOYEE
