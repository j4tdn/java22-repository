...
public class WhAllocationDemo {
	...
	public static void main(String[] args) {
		...
		if (planningAmount > requiredMinPlanningAmount) {
			...
			// step3
			Map<Integer, BigDecimal> mapStoreDemands = calculateStoreDemand(mapStorePotential55, mapStorePotential77,
					refWeights, trendStore);
	...
	private static Map<Integer, BigDecimal> calculateSumDemand(Map<Integer, BigDecimal> mapStoreDemand, List<Store> ListStoresOfA55 ){
	    ...
	    Set<Integer> level =  ListStoresOfA55.stream().map(Store::getWhId).collect(Collectors.toSet());
	    for(Integer wh: level) {
            ...
            // Lưu tổng demand vào kết quả
            result.put(wh, sumDemand);
	    }
	    return result;
	}

	private static Map<Integer, BigDecimal> calculateStoreDemand(Map<Integer, BigDecimal> mapStore55,
			Map<Integer, BigDecimal> mapStore77, Map<Integer, BigDecimal> refWeights,
			Map<Integer, BigDecimal> trendStore) {

		Map<Integer, BigDecimal> result = new HashMap<>();

		// Tính tổng giá trị từ refWeights
		BigDecimal sumOfValues = refWeights.values().stream().reduce(BigDecimal.ZERO, BigDecimal::add);
		...
		for (Integer key : mapStore55.keySet()) {
			...
			// Tính toán nhu cầu
			BigDecimal demand = (value55.multiply(weight55).add(value77.multiply(weight77)))
					.divide(sumOfValues, RoundingMode.HALF_UP).multiply(trend);
	...
	private static Map<Integer, BigDecimal> fillMissingStorePotential(List<Store> ListStore,
			Map<Integer, Integer> refStore) {
...
//		System.out.println("average -->"+ average +",,,,"+ MapStore.size());
		// cập nhật
		ListStore.stream().filter(store -> store.getPotential().compareTo(BigDecimal.ZERO) == 0) // lọc các store cần
																									// fill
				.forEach(store -> {
