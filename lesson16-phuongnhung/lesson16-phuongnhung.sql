...
WITH CTE_ITEM_PRICE AS (
	SELECT C03_ITEM_ID ITEM_ID, 
       MAX(C03_SALES_PRICE) MAX_SALE_PRICE
    FROM T03_ITEM_DETAIL
    GROUP BY C03_ITEM_ID
)
SELECT * 
  FROM T01_ITEM t01
  JOIN CTE_ITEM_PRICE cte
	 on t01.C01_ITEM_ID = cte.ITEM_ID
ORDER BY cte.MAX_SALE_PRICE DESC
LIMIT 5;
...
WHERE cast(C04_ORDER_TIME AS DATE)
	BETWEEN str_to_date('28/10/2024', '%d/%m/%Y')
...
SELECT C01_ITEM_ID ITEM_ID,
       ...
       C03_SALES_PRICE SALES_PRICE,
       ROUND(C03_SALES_PRICE * (100-20)/100, 2) AFTER_SALES_PRICE
  FROM T01_ITEM t01
...
SELECT C01_ITEM_ID,
       ...
       C03_ITEM_DETAIL_ID,
       C03_SALES_PRICE
FROM t03_item_detail t03, t01_item t01
WHERE t03.C03_ITEM_ID = t01.C01_ITEM_ID
...
WHERE cast(C04_ORDER_TIME AS DATE) = str_to_date('27/10/2024', '%d/%m/%Y')
	OR cast(C04_ORDER_TIME AS DATE) =  str_to_date('29/10/2024', '%d/%m/%Y');
...
SELECT * 
  ...
  JOIN t03_item_detail t03
	 on t01.C01_ITEM_ID = t03.C03_ITEM_ID
ORDER BY t03.C03_SALES_PRICE ASC;
