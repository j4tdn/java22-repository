...
public class HibernateItemGroupDao extends GenericDao implements ItemGroupDao {
	...
	private static final String Q_GET_ITEM_GROUP_DETAILS = "" + "SELECT t2.C02_ITEM_GROUP_ID " + ItemGroupDto.PROP_ID
			+ ",\n" + "	   t2.C02_ITEM_GROUP_NAME " + ItemGroupDto.PROP_NAME + ",\n" + "	   COUNT(*) "
			+ ItemGroupDto.PROP_AMOUNT_OF_ITEMS + ",\n" + "       GROUP_CONCAT(C01_ITEM_NAME SEPARATOR ', ') "
			+ ItemGroupDto.PROP_DETAILS + "\n" + " FROM t01_item t1\n"
			+ " JOIN t02_item_group t2 ON t1.C01_ITEM_GROUP_ID = t2.C02_ITEM_GROUP_ID\n"
			+ " GROUP BY C01_ITEM_GROUP_ID";

	private static final String Q_GET_ITEM_GROUP_BY_NAME = "" + "SELECT *" + " FROM T02_ITEM_GROUP"
			+ " WHERE C02_ITEM_GROUP_NAME = " + SQL_PARAM_IG_NAME;

	private static final String Q_DELETE_ITEM_GROUP_BY_ID = "" + "DELETE FROM T02_ITEM_GROUP WHERE C02_ITEM_GROUP_ID = "
			+ SQL_PARAM_IG_ID;	

	/*
	 * với các thao tác liên quan đến câp nhật dữ liệu THÊM - SỬA - XÓA
	 * 
	 * Luôn cần transaction Có 2 cách làm 1. Dùng các hàm đơn giản có sẵn của Java
	 * với Entity 2. Tự viết truy vấn
	 */
	...
	public void testHibernateCache() {
		...
		var group2 = session1.get(getEntityClass(), 20); // database
		...
		var group4 = session2.get(getEntityClass(), 20); // 2nd level cache, database
	...
	public void delete(Integer groupId) {
		...
		/*
		 * execute(session -> { var entity = session.get(getEntityClass(), groupId);
	...
	public List<ItemGroup> getAll() {
		return openSession().createNamedQuery(ItemGroup.Q_GET_ALL, getEntityClass()).getResultList();
	}
	...
	public List<ItemGroupDto> statisticItemGroupDetails() {
		 ...
		 * .createNamedQuery(Q_GET_ITEM_GROUP_DETAILS) .getResultList();
		 * 
		 * return rawList.stream() .map(this::transformToItemGroupDto) .toList();
		 */

		return openSession().createNativeQuery(Q_GET_ITEM_GROUP_DETAILS)
	...
	public ItemGroup get(String name) {
		return openSession().createNativeQuery(Q_GET_ITEM_GROUP_BY_NAME, getEntityClass())
				.setParameter(PARAM_IG_NAME, name, StandardBasicTypes.STRING).uniqueResult(); // getSingleResult
	}
	...
	public ItemGroupDto transformToItemGroupDto(Object[] object) {
		return new ItemGroupDto(Integer.parseInt(String.valueOf(object[0])), String.valueOf(object[1]),
				Integer.parseInt(String.valueOf(object[2])), String.valueOf(object[3]));
	}
