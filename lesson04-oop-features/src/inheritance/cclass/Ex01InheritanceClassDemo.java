...
public class Ex01InheritanceClassDemo {
	public static void main(String[] args) {
		...
		/*
		 * Bài toán: Viết chương trình quản lý lưu trữ sách trong nhà sách Trong đó quản
		 * lý 2 loại sách chính:
		 * 
		 * Sách giáo khoa: mã sách, tên sách, giá bán, nxb, tình trạng, %giảm giá(nếu
		 * sách cũ) Sách tham khảo: mã sách, tên sách, giá bán, nxb, %thuế
		 * 
		 * Book: id, name, salesPrice, publisher
		 * 
		 * TextBook extends Book: status, discount ReferenceBook extends Book: tax
		 * 
		 * Hệ thống có rất nhiều sách, ứng dụng để tìm kiểm, tính toán dễ hơn
		 * 
		 * Câu 1: Liệt kê các cuốn sách thuộc nhà xuất bản Nhi Đồng Câu 2: Liệt kê các
		 * cuốn sách được giảm giá Câu 3: Giải lập việc mua 1 vài cuốn sách và tính tổng
		 * tiền
		 * 
		 * Sách giáo khoa: số tiền = salesPrice(100 - %discount)/100 Sách tham khảo: số
		 * tiền = salesPrice(100 + %tax)/100
		 */
	...
	private static BigDecimal calcTotalOfMoney(Book[] selectedBooks) {
		...
		for (Book book : selectedBooks) {
			...
			if (book instanceof TextBook) {
				...
				price = price.multiply(new BigDecimal("100").add(tax)).divide(new BigDecimal("100"), 10,
	...
	private static Book[] getBooksWithDiscount(Book[] books) {
		...
		for (Book book : books) {
			...
			if (book != null && book.getClass() == TextBook.class) {
				...
				// muốn ép biến từ KDL cha sang con thì phải kiểm tra biến cha đó lúc runtime
				// chính xác là con rồi ép
				TextBook tb = (TextBook) book;
