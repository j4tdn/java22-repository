...
public class Ex01BubbleSortInt {
	...
	public static void main(String[] args) {
		...
		/*
		 ...
		 Đúng: nếu tăng dần trước < sau
			 : 	   giảm 		  >
		 + cứ sau mỗi round là lại được 1 phần tử lớn, nhỏ nhất nằm bên phải, trái tùy cách code
	...
	// nếu kết quả của compare(element[j], elements[j+1]) trả về thỏa mãn > 0
	// hoán vị
	private static void bubbleSort(int[] elements,CompareInt cbInt) {
		int length = elements.length;
		for(int i = 0; i < length; i++) {
			for(int j = 0; j < length - i - 1; j++) {
				if(cbInt.compare(elements[j], elements[j+1]) > 0) {
					int temp = elements[j];
