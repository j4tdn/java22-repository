...
public class Ex02SortItem {
	...
	public static void main(String[] args) {
		...
		/*
		    ...
		    Cách 2: Arrays.sort(Object[] objects)
		            ...
		            + Lúc runtime(truyền vào KDL con của Object) thì từng 'đối tượng' phải là con
		              ...
		              --> sau đó muốn sort tăng, giảm dần thì phải overide hàm compareTo của Comparable
		                  trong class
		    ...
		    Với cách 2 thì với 1 mảng đối tượng mình sort theo được 1 cách, phương pháp duy nhất
		    vì muốn sort thế nào phải override logic của hàm compareTo .... 1 style
		            
		 */
		// Cách 1
		Arrays.sort(items, (i1, i2) ->  i1.getId().compareTo(i2.getId()));
		SortUtil.printf("1. Sắp xếp tăng dần theo id", items);
		
		Arrays.sort(items, (i1, i2) ->  i2.getSalesPrice().compareTo(i1.getSalesPrice()));
		SortUtil.printf("2. Giảm dần theo salesPrice", items);
		...
		SortUtil.printf("3. Giảm dần theo salesPrice, tăng dần theo salesDate", items);
		...
		SortUtil.printf("4. Null Last, tăng dần theo name", itemsWithNullObjects);
		
		System.out.println("\n=============== Cách 2(Arrays.sort(Object[] objects)) =============== \n");
		
		Arrays.sort(items);
		SortUtil.printf("1. Tăng dần theo salesPrice", items);
		
		Arrays.sort(items);
		SortUtil.printf("2. Giảm dần theo id", items);
