Đã dùng 95% bộ nhớ … Nếu hết dung lượng lưu trữ, bạn sẽ không thể tạo, chỉnh sửa và tải tệp lên. Sử dụng 100 GB dung lượng với giá 45.000 ₫ 0 ₫ trong 1 tháng.
-- A. INSERT
...
-- 2. Đơn hàng
    ...
    SELECT * 
		FROM t04_order
        WHERE DATE(C04_ORDER_TIME) = '2024-10-27';
	-- CÁCH 2
    ...
    -- Được bán từ ngày 27/10/2024 đến ngày 29/10/2024
    SELECT * 
		FROM t04_order
		WHERE CAST(C04_ORDER_TIME AS DATE) 
			  BETWEEN STR_TO_DATE('27/10/2024', '%d/%m/%Y') 
			  AND STR_TO_DATE('29/10/2024', '%d/%m/%Y');

    -- Được bán trong tháng 10/2024
    SELECT * 
		FROM t04_order
    ...
    SELECT * FROM t04_order WHERE C04_DELIVERY_ADDRESS = 'Địa chỉ X1';
    
-- 3. Giá của toàn bộ các mặt hàng sau khi được khuyến mãi 20%, làm tròn 2 chữ số thập phân
	SELECT  C01_ITEM_ID c01_id,
			C01_ITEM_NAME c01_name,
			C03_SALES_PRICE c03_sale_price,
			ROUND(C03_SALES_PRICE * 0.80, 2) AS Discounted_Price
		FROM t01_item t01
		JOIN t03_item_detail t03 
        ON   t01.C01_ITEM_ID = t03.C03_ITEM_ID;

-- 4. Giảm giá 20% tất cả các mặt hàng trong ngày 27/10/2024
	SELECT C01_ITEM_ID, C01_ITEM_NAME, C04_ORDER_TIME,
		ROUND(C03_SALES_PRICE * 0.80) AS Discounted_Price
		FROM t01_item t01, t04_order t04, t03_item_detail t03, t05_order_detail t05
		WHERE t03.C03_ITEM_DETAIL_ID = t05.C05_ITEM_DETAIL_ID
		AND   t04.C04_ORDER_ID = t05.C05_ORDER_ID
