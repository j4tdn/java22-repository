...
public class Ex01BasicDemo {
	...
	/* 
	 ...
	 . HashMap  	 : ko đảm bảo thứ tự đưa vào, ko sắp xếp, được phép chứa key = NULL
	 . LinkedHashMap : đảm bảo thứ tự đưa vào
...
//		Map<String, Integer> models = new LinkedHashMap<>();
		...
		Map<String, Integer> sortedModels = sort(models, (e1, e2) -> {
			...
			return v1.compareTo(v2); //accesding
		});
//		LoopByEntry("2. Loop map by sorted entries", sortedModels);
		LoopByKey("2. Loop map by key", models);
		
		// hỗ trợ tìm kiếm theo key = Quảng Trị
		System.out.println("Quảng Trị có biển số xe = " + models.get("Quảng Trị"));
	}
	
	private static Map<String, Integer> sort(Map<String, Integer> models, Comparator<Entry<String, Integer>> comparator){
		// B1: chuyển đổi từ Map -> Set<Entry<> -> List<Entry<K,V> để trong List hỗ trợ hàm sort
