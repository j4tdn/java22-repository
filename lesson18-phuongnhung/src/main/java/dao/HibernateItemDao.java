...
public class HibernateItemDao extends GenericDao implements ItemDao {
	...
	private static final String Q_GET_TOP_3_SELLING_ITEMS = ""
	        + "SELECT C01_ITEM_ID " + ItemDto.PROP_ID + ",\n"
	        + "       C01_ITEM_NAME " + ItemDto.PROP_NAME + ",\n"
	        + "       SUM(C05_AMOUNT) AS " + ItemDto.PROP_TOTAL_QUANTITY + "\n"
	        + "FROM t01_item t1\n"
	        ...
	        + "WHERE YEAR(t4.C04_ORDER_TIME) = :yearParam\n"
	        + "GROUP BY C01_ITEM_ID, C01_ITEM_NAME\n"
	        + "ORDER BY TOTAL_QUANTITY DESC, C01_ITEM_ID DESC\n"
	        + "LIMIT 3;";
	...
	public List<ItemDto> getTop3SellingItems(int year) {
	    return openSession()
	            .createNativeQuery(Q_GET_TOP_3_SELLING_ITEMS)
	            .setParameter("yearParam", year)
	            .addScalar(ItemDto.PROP_NAME, StandardBasicTypes.STRING)
	            .addScalar(ItemDto.PROP_TOTAL_QUANTITY, StandardBasicTypes.INTEGER)
	            .setResultTransformer(Transformers.aliasToBean(ItemDto.class))
	            .getResultList();
	}
