...
public class WhAllocationDemo {
    ...
    public static void main(String[] args) {
        // Step 1: Kiểm tra "Planning Amount" (Số lượng mặt hàng cần phân bổ)
        boolean shouldContinue = checkPlanningAmount(
            DataModel.planningAmount,
            DataModel.requiredMinPlanningAmount
        );
        if (shouldContinue) {
            // Step 2: Điền giá trị thiếu bằng cách tham chiếu hoặc lấy trung bình
            ...
            // Step 3: Tính toán nhu cầu của Store cho Item hiện tại (Item A1) 
            ...
            calculateStoreDemand(storePotentialA55.get(new Item(55, "Item A55")), 
                                 storesA77,  
                                 refWeights);

            // Step 4: Tổng hợp Nhu cầu lên cấp WH
            System.out.println("Step 4!!");
            Map<Integer, BigDecimal> whDemand = sumUpDemandToWH(storesA77);
            whDemand.forEach((whId, demand) -> 
                System.out.println("WH " + whId + ": " + demand));

            // Step 5: Tính toán tỷ lệ phân bổ của từng kho hàng
    ...
    private static void calculateStoreDemand(List<Store> storesA55, List<Store> storesA77, Map<Integer, BigDecimal> refWeights) {
        for (int i = 0; i < storesA55.size(); i++) {
            ...
            BigDecimal demand = (potentialA55.multiply(weightA55).add(potentialA77.multiply(weightA77)))
                    .divide(weightA55.add(weightA77), RoundingMode.HALF_UP)
                    .multiply(storeTrend);
    ...
    private static void calculateShares(Map<Integer, BigDecimal> whDemand) {
        BigDecimal totalDemand = whDemand.values().stream()
                                         .reduce(BigDecimal.ZERO, BigDecimal::add);

        if (totalDemand.compareTo(BigDecimal.ZERO) > 0) {
            whDemand.forEach((whId, demand) -> {
                BigDecimal share = demand.multiply(BigDecimal.valueOf(100))
                                         .divide(totalDemand, RoundingMode.HALF_UP);
